...
![test](../images/Chapter_0/image.png)
...
./scripts/config --enable CONFIG_STACKPROTECTOR_NONE
...
![image](../images/Chapter_0/image-1.png)
...
![busybox](../images/Chapter_0/image-2.png)
...
![rootfs](../images/Chapter_0/image-3.png)
...
여기서 init에 주목하세요. 이 프로세스는 리눅스가 부팅된 이후 가장 먼저 실행되며, 모든 프로세스의 부모인 1번 프로세스가 됩니다.
...
![alt text](../images/Chapter_0/image-4.png)

실행 결과 다음 사진처럼 rootfs\.cpio 가 kernel 디렉토리에 보인다면 잘 하신 겁니다!

**앞으로 모든 커널 실습에서 이 rootfs 디렉터리는 환경 구축에 계속 사용할 것입니다.**

### 잠깐! /proc/kallsyms 파일은 무엇인가요?

이 파일은 쉽게 말해 **커널 내부의 거대한 '주소록(전화번호부)' 같은 역할을 합니다.** 커널에 존재하는 수많은 함수와 변수들(이를 심볼, Symbol이라 부릅니다)이 현재 메모리의 어느 위치(주소)에 적재되어 있는지를 전부 기록해 둔 파일입니다.

앞서 우리는 운영체제가 해킹을 당할 경우 공격자가 커널 전체를 제어할 수 있다고 배웠습니다. 하지만 공격자가 제어를 하려면 실행하고자 하는 함수가 메모리의 몇 번지에 있는지 정확한 시작 주소를 알아야겠죠? 이 `/proc/kallsyms` 파일에는 **운영체제 커널(vmlinux) 바이너리에 정의된 함수들의 실제 주소들이 담겨 있습니다.**

🚨 보안 팁: 사실 현대 리눅스에서는 해커들이 이 주소록을 보고 커널을 공격하지 못하도록, 일반 사용자가 이 파일을 읽으면 주소가 전부 `0000000000000000`으로 가려져 보이도록 막아두었습니다. (이를 kptr\_restrict 보호 기법이라 합니다.)

하지만 실습 환경에서는 여러분이 공격의 기본 원리를 쉽게 이해할 수 있도록, 일반 사용자도 진짜 주소를 볼 수 있게 만든 복사본을 `/tmp/kallsyms`에 미리 준비해 두었습니다.

나중에는 이렇게 친절하게 주어진 파일 없이도, 고급 해킹 기법을 사용해 숨겨진 커널 주소를 직접 알아내는(Memory Leak) 방법도 배우게 될 것입니다!
...
이제 모든 준비가 끝났습니다! 이제 우리가 위에서 만든 커널 이미지, 루트 파일 시스템을 QEMU를 사용해서 실행할 시간입니다.

우선 아래의 명령어를 사용하여, `linux-` 디렉토리 안에 저장된 커널 이미지를 복사합니다.(루트 파일 시스템은 위에서 이미 하셨던 것 기억 나시죠?)
...
![result](../images/Chapter_0/image-6.png)

이제 커널 환경을 구동해 봅시다! `sudo ./qemu.sh`를 입력하여 QEMU를 실행하세요!(하드웨어 가속에 관리자 권한이 필요합니다.)

![boot](../images/Chapter_0/image-5.png)

새 터미널 창이 부여지고, id, ls 명령어가 위 사진처럼 잘 실행되면 성공입니다. **QEMU에서 나오려면 Ctrl+A 를 누른 이후 X 키를 눌러 빠져나오세요.**
