...
먼저 실습 자료 폴더(`src/sample`) 안에 있는 `first_chall.zip` 파일을 다운로드하여 압축을 풀어주세요.
...
ssize_t __fastcall dev_write(file *filep, const char *buffer, size_t len, loff_t *offset)
{
  ...
  v6 = _copy_from_user(a1: k_buffer, a2: buffer, a3: len) == 0;
  result = -14;
  if ( v6 )
    return len;
  return result;
...
그렇다면 도대체 쓰레기(Dummy) 데이터를 몇 바이트나 채워 넣어야 반환 주소를 덮을 수 있을까요? 이를 정확히 계산하기 위하여, IDA View에서 함수가 끝나는 부분(에필로그)를 살펴봅시다. 코드가 종료될 때 스택 메모리가 어떻게 정리되는지 주목하세요.
